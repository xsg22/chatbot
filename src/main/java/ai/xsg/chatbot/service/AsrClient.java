...
public class AsrClient extends AbstractWebSocketHandler {
    ...
    public void init() {
        ...
        WebSocketSession webSocketSession = webSocketClient.execute(new AbstractWebSocketHandler() {
            protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
                System.out.println("get text message");
            }

            protected void handleBinaryMessage(WebSocketSession session, BinaryMessage message) throws Exception {
                System.out.println("get binary message");
                try {
                    if (parse_response(message.getPayload()) != 0) {
                        recv_suc = false;
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        logger.info("onMessage String, {}", message.getPayload());
    }
...
//                close();
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        logger.info("asr onError {}", exception.getMessage());
        recv_suc = false;
        recv_latch.countDown();
    }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        logger.info("asr onClose {}, {}", status.getCode(),status.getReason());
    }
...
//    }
    ...
    public boolean asr_sync_connect() throws IOException, InterruptedException, NoSuchAlgorithmException, InvalidKeyException {
        this.params_msg = construct_param();
        boolean ret = this.webSocketSession.isOpen();
        if (!ret) {
            return ret;
        }
        recv_latch = new CountDownLatch(1);
        this.webSocketSession.sendMessage(new BinaryMessage(this.params_msg));
        ret = recv_latch.await(recv_timeout, TimeUnit.SECONDS);
        return ret && recv_suc;
    }

    public void sendParamsMsg() throws IOException, InterruptedException {
        this.params_msg = construct_param();
        recv_latch = new CountDownLatch(1);
        this.webSocketSession.sendMessage(new BinaryMessage(this.params_msg));
    ...
    public AsrResponse asr_send(byte[] audio, boolean is_last) throws IOException, InterruptedException {
        ...
        this.webSocketSession.sendMessage(new BinaryMessage(payload));
        boolean ret = recv_latch.await(recv_timeout, TimeUnit.SECONDS);
        if (!ret) {
    ...
    private void set_auth_header(WebSocketHttpHeaders headers) throws NoSuchAlgorithmException, InvalidKeyException {
        ...
        headers.add(custom_header, custom_cont);

        String str = "GET " + webSocketSession.getUri().getPath() + " HTTP/1.1\n"
                + custom_cont + "\n";
        ...
        String auth_cont = "HMAC256; access_token=\"" + this.token
                + "\"; mac=\"" + base64_data
                + "\"; h=\"" + custom_header + "\"";
        headers.add("Authorization", auth_cont);
    }
